...
const loadDictionary = async (): Promise<Set<string>> => {
  try {
    ...
    if (!dicResponse.ok) {
      throw new Error(`HTTP error! status: ${dicResponse.status}`)
    }
...
export class SpellChecker {
  ...
  static async checkWord(word: string): Promise<SpellCheckResult> {
    ...
    try {
      ...
      if (word.toLowerCase() === 'wulki') {
        console.log('Checking word "wulki":', {
          originalWord: word,
          cleanWord,
          isCorrect,
          dictionarySize: UPPER_SORBIAN_DICTIONARY.size,
          hasWord: UPPER_SORBIAN_DICTIONARY.has(cleanWord),
          sampleWords: Array.from(UPPER_SORBIAN_DICTIONARY).slice(0, 10),
        })
      }
